...
export namespace Aufgabe3_4 {
    ...
    let databaseUrl: string = "mongodb+srv://Uwu-Nutzer:UWUPASSWORT1234@gis-ist-geil.uvkck.mongodb.net/Test?retryWrites=true&w=majority"; // Port der Datenbank
    ...
    async function handleRequest(_request: Http.IncomingMessage, _response: Http.ServerResponse): Promise<void> { // wird immer gesetzt 
        ...
        switch (requestUrl.pathname) { // wird nach dem pathname abgefragt --> ist in requestUrl gespeichert
            case "/insert": //geht in die DB 
                ...
                let student: Student = { name: urlQuery["name"].toString(), firstname: urlQuery["firstname"].toString(), matrikelnummer: Number(urlQuery["matrikelnummer"].toString()) }; // asso Array um die Date zu finden und in eine neue Var eingefügt
                students.insertOne(student); // Ein Student wird hinz. 
                break;
            case "/get": //holt die Daten von der DB
                let cursor: Mongo.Cursor = students.find(); // der Cursor greif auf alle gespeicherten Studenten zu
                let studentsData: Student[] = await cursor.toArray(); // cursor.toArray() --> konvertiert den Cursor zu einem Array
    ...
    async function conntectToDB(_url: string): Promise<void> { // nur dazu da mit der DB zu verbinden
